// PngTail.cpp - saves flow data as PNG
...
void* PngTail::save(Flow& flow) {
    ...
    // Convert float RGBA (0-1) to uint8 RGB
    ...
    // Encode PNG
    std::vector<uint8_t> png = encodePng(rgb.data(), width, height);

    // Write to file
    std::ofstream file(path_, std::ios::binary);
    if (!file) {
        std::cerr << "PngTail: failed to open " << path_ << "\n";
        return nullptr;
    }

    file.write(reinterpret_cast<char*>(png.data()), png.size());
    file.close();
