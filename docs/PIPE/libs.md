...
PIPE exposes a static library (`pipe.a`) with public headers. The GEAR decoder is compiled into `pipe.a`. OpenCV is dynamically linked at runtime, so consumers link against `pipe.a` and set `LD_LIBRARY_PATH` for OpenCV.
...
The static library containing all PIPE functionality.
...
# Headers
INCLUDES = -I$(PIPE_DIR)/inc \
           -I$(PIPE_DIR)/src/main/part/pipe

# Library (static)
LIBS = $(PIPE_DIR)/lib/pipe.a

# OpenCV (runtime)
# LD_LIBRARY_PATH=$(PIPE_DIR)/lib/opencv/build/lib ./your_app
```
...
Sony decoder compiled into `pipe.a` from `lib/GEAR.a`.
...
LIBS = $(PIPE_DIR)/lib/pipe.a \
       $(PIPE_DIR)/lib/opencv/build/lib/libopencv_core.so \
       $(PIPE_DIR)/lib/opencv/build/lib/libopencv_imgproc.so \
       $(PIPE_DIR)/lib/opencv/build/lib/libopencv_imgcodecs.so
```
