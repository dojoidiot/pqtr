...
void filmicrgb_autotune(FilmicRGBData *data, const float *pixels, int width, int height)
{
    ...
    fprintf(stderr, "filmicrgb_autotune: min_rgb=[%.10e,%.10e,%.10e] max_rgb=[%.10e,%.10e,%.10e] black=%.10e white=%.10e grey=%.10e EVmin=%.10e EVmax=%.10e\n",
            min_rgb[0], min_rgb[1], min_rgb[2], max_rgb[0], max_rgb[1], max_rgb[2],
            black, white, grey, EVmin, EVmax);
    fprintf(stderr, "filmicrgb_autotune: black_ev=%.10e white_ev=%.10e DR=%.10e output_power=%.10e\n",
