    ...
    /* Note: OpenMP parallelization removed for simplicity - single threaded */
    {
        ...
        for(int tile_vertical = 0; tile_vertical < num_vertical; tile_vertical++)
        {
            for(int tile_horizontal = 0; tile_horizontal < num_horizontal; tile_horizontal++)
            {
                ...
                memset(PQ_Dir, 0, sizeof(float) * DT_RCD_TILESIZE * DT_RCD_TILESIZE / 2);
                memset(cfa, 0, sizeof(float) * DT_RCD_TILESIZE * DT_RCD_TILESIZE);
                memset(P_CDiff_Hpf, 0, sizeof(float) * DT_RCD_TILESIZE * DT_RCD_TILESIZE / 2);
                memset(Q_CDiff_Hpf, 0, sizeof(float) * DT_RCD_TILESIZE * DT_RCD_TILESIZE / 2);
