...
#include "lib/pipe_prepare.c"

/* All modules (types inlined into each module) */
#include "mods/rawprepare.c"
#include "mods/temperature.c"
#include "mods/highlights.c"
#include "mods/demosaic.c"
#include "mods/exposure.c"
#include "mods/colorin.c"
#include "mods/channelmixerrgb.c"
#include "mods/filmicrgb.c"
#include "mods/colorbalancergb.c"
#include "mods/bilat.c"
#include "mods/colorout.c"
#include "mods/autotune.c"
#include "mods/denoiseprofile.c"
...
{
    const char* input_path = "src/test/raws/DSC00458.ARW";
    const char* output_path = "tmp/out.png";

    if (argc >= 2) input_path = argv[1];
    if (argc >= 3) output_path = argv[2];

    printf("=== Args Pipeline ===\n");
    printf("Input:  %s\n", input_path);
    printf("Output: %s\n", output_path);
    ...
    dump_buffer_u16("00_decode", bayer_u16, width, height, 1);
    free(compressed);
    ...
    dump_buffer("01_rawprepare", bayer_f32, width, height, 1);
    ...
    dump_buffer("02_temperature", bayer_wb, width, height, 1);
    free(bayer_f32);
    ...
    dump_buffer("03_highlights", bayer_hl, width, height, 1);
    free(bayer_wb);
    ...
    dump_buffer("04_demosaic", rgb, width, height, 4);
    free(bayer_hl);
    ...
    dump_buffer("05_exposure", rgb_exp, width, height, 4);
    free(rgb);
    ...
    dump_buffer("06_colorin", rec2020, width, height, 4);
    free(rgb_exp);
    ...
    dump_buffer("07_channelmixerrgb", rec2020_cm, width, height, 4);
    free(rec2020);
    ...
    /* Get ISO from EXIF metadata (parsed by sony.c) */
    int iso = meta.iso > 0 ? meta.iso : 100;
    printf("   ISO: %d\n", iso);
    ...
    dump_buffer("08_denoiseprofile", rec2020_dn, width, height, 4);
    free(rec2020_cm);
    ...
    dump_buffer("09_colorbalancergb", rec2020_cb, width, height, 4);
    free(rec2020_dn);
    ...
    dump_buffer("10_filmicrgb", rec2020_filmic, width, height, 4);
    free(rec2020_cb);
    ...
    dump_buffer("11_bilat", rec2020_bilat, width, height, 4);
    free(rec2020_filmic);
    ...
    dump_buffer("12_colorout", srgb, width, height, 4);
    free(rec2020_bilat);

    /* ======================================================================
       15. Write PNG
       ====================================================================== */
    printf("13. Writing PNG...\n");
    ...
    /* ======================================================================
       16. Auto-tune: compare to embedded JPEG, report optimal exposure
       ====================================================================== */
    printf("14. autotune...\n");
    float current_ev = meta.camera ? meta.camera->style.exposure_ev : 0.0f;
    float ev_adjustment = raw_exposure_autotune(input_path, meta.preview_offset,
                                                 meta.preview_length, png_data, width, height);
