...
typedef struct {
    ...
    int iso;                 /* ISO speed from EXIF */
    uint32_t preview_offset; /* Embedded JPEG preview offset */
    uint32_t preview_length; /* Embedded JPEG preview length */
} SonyARWMeta;
...
/* Map Sony Picture Profile (from tag 0x0237) to saturation/vibrance values
   ...
   Picture Profile values (reference: Sony.pm documentation):
...
/* Sony encryption LFSR (Dave Coffin's algorithm from dcraw)
   Used for both SR2SubIFD and Tag 0x2010 */
static void sony_decrypt(uint8_t* data, uint32_t length, uint32_t key)
{
    uint32_t pad[128] = {0};
    uint32_t p;
...
int sony_arw_read_meta(const char* filename, SonyARWMeta* meta)
{
    ...
    for (int i = 0; i < nentries; i++) {
        ...
        /* JPEG preview: tags 0x0201 (offset) and 0x0202 (length) */
    ...
    /* Parse EXIF IFD for MakerNotes and ISO */
    ...
    if (exif_ifd_offset > 0 && exif_ifd_offset + 2 <= read_size) {
        ...
        for (int i = 0; i < exif_nentries; i++) {
            ...
            /* ISO speed: tag 34855 (0x8827), type SHORT (3) */
            if (entry.tag == 34855) {
                if (entry.type == 3) { /* SHORT */
                    meta->iso = entry.value_offset & 0xFFFF;
                } else if (entry.type == 4) { /* LONG */
                    meta->iso = entry.value_offset;
                }
            }
            offset += 12;
        }
    ...
    if (maker_note_offset > 0 && maker_note_offset + 12 <= read_size) {
        ...
        if (maker_ifd_offset + 2 <= read_size) {
            ...
            /* Parse tag 0x2010 - encrypted Sony MakerNote sub-IFD
               Contains: PictureProfile (0x201a), DynamicRangeOptimizer (0x2016), ToneCurve (0x7010) */
            if (sony_tag2010_offset > 0 && sony_tag2010_count > 4 &&
                sony_tag2010_offset + sony_tag2010_count <= read_size) {

                /* Allocate and copy tag data */
                uint8_t* tag2010 = (uint8_t*)malloc(sony_tag2010_count);
                if (tag2010) {
                    memcpy(tag2010, data + sony_tag2010_offset, sony_tag2010_count);

                    /* Key is first 4 bytes, decrypt remainder
                       Note: This decryption doesn't work without camera-model-specific keys.
                       We use exiftool for PictureProfile/DRO instead, but keep this code
                       for the tone curve which uses a simpler format. */
                    uint32_t key = meta_read_u32(tag2010);
                    sony_decrypt(tag2010 + 4, sony_tag2010_count - 4, key);

                    /* Decrypted data is IFD starting at byte 4 */
                    uint8_t* dec = tag2010 + 4;
                    uint32_t dec_len = sony_tag2010_count - 4;

                    if (dec_len >= 2) {
                        uint16_t tag2010_nentries = meta_read_u16(dec);

                        for (int i = 0; i < tag2010_nentries && i < 200; i++) {
                            ...
                            if (entry.tag == 0x7010 && entry.count >= 4) {
                                uint32_t curve_off = entry.value_offset;
                                /* Value may be offset relative to tag2010 or embedded */
                                if (curve_off < dec_len && curve_off + 8 <= dec_len) {
                                    for (int j = 0; j < 4; j++) {
                                        meta->sony_curve[j] = meta_read_u16(dec + curve_off + j * 2);
                                    }
                                    found_sony_curve = 1;
                                }
                            }
    ...
    if (sr2_offset > 0 && sr2_length > 0 && sr2_key != 0 &&
        ...
        if (sr2) {
            ...
            sony_decrypt(sr2, sr2_length, sr2_key);
    ...
    /* Try to get PictureProfile and DRO from exiftool (if available)
       Tag 0x2010 is encrypted with camera-model-specific keys that require exiftool's
       decryption tables. If exiftool isn't available, we fall back to neutral values. */
    {
        char cmd[512];
    ...
    /* Map Picture Profile to saturation/vibrance */
    ...
    /* Debug output */
    const char* pp_names[] = {
        "Off", "Portrait", "Standard", "Cinema", "Cine1-SGamut", "Cine2-SGamut",
        "Cine1-Cinema", "S-Log2", "Vivid", "S-Log3-Cine", "S-Log3"
    };
    ...
    printf("sony: PictureProfile: %d (%s), sat=%.2f, vib=%.2f\n",
           meta->profile.picture_profile, pp_name,
           meta->profile.saturation, meta->profile.vibrance);
    printf("sony: DRO: %d (%s), shadow_lift=%.2f\n",
           meta->dro_level, dro_name, meta->dro_shadow_lift);
    printf("sony: ISO: %d, preview: offset=%u len=%u\n",
           meta->iso, meta->preview_offset, meta->preview_length);
