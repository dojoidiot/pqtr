...
{
    dims_ = {width, height};
    buffer_size_ = width * height * 3 * sizeof(float);

    // Create input buffer (storage, copy_dst)
    wgpu::BufferDescriptor input_desc{};
    input_desc.size = buffer_size_;
...
bool Pipe::dispatch()
{
    ...
    output_entry.size = buffer_size_;
    ...
    encoder.CopyBufferToBuffer(output_buffer_, 0, staging_buffer_, 0, buffer_size_);
...
std::vector<float> Pipe::get_output()
{
    ...
    result.resize(dims_.width * dims_.height * 3);

    bool done = false;
    staging_buffer_.MapAsync(
        wgpu::MapMode::Read, 0, buffer_size_,
        wgpu::CallbackMode::AllowSpontaneous,
        [&](wgpu::MapAsyncStatus status, wgpu::StringView) {
            if (status == wgpu::MapAsyncStatus::Success)
            {
                const void* mapped = staging_buffer_.GetConstMappedRange(0, buffer_size_);
                memcpy(result.data(), mapped, buffer_size_);
