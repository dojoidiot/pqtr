...
/* ============================================================================
   ...
   Uses exiftool for simplicity (could be replaced with direct EXIF parsing).
   ============================================================================ */
...
{
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "exiftool -b -PreviewImage \"%s\" > \"%s\" 2>/dev/null",
             raw_path, out_jpeg_path);

    int ret = system(cmd);
    if (ret != 0) {
        fprintf(stderr, "autotune: failed to extract embedded JPEG from %s\n", raw_path);
        return -1;
    }

    /* Verify output file exists and is valid JPEG */
    FILE* f = fopen(out_jpeg_path, "rb");
    if (!f) return -1;

    unsigned char header[2];
    if (fread(header, 1, 2, f) != 2 || header[0] != 0xFF || header[1] != 0xD8) {
        fclose(f);
        fprintf(stderr, "autotune: extracted file is not valid JPEG\n");
        return -1;
    }
    fclose(f);
...
/*
    ...
    1. Extract embedded JPEG from RAW
                            ...
                            int width, int height)
{
    ...
    if (extract_embedded_jpeg(raw_path, tmp_jpeg) != 0) {
