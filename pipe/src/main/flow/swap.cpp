...
std::vector<uint8_t> swap(const void *data, int w, int h, Swap from, Swap into)
{
    ...
    // Transcode compressed -> compressed (w = byte size)
    if ((from == JPG || from == PNG) && (into == JPG || into == PNG))
    {
        ImageResult decoded;
        if (from == JPG)
            decoded = decodeJpeg(bytes, static_cast<size_t>(w));
        else
            decoded = decodePng(bytes, static_cast<size_t>(w));

        if (decoded.rgb.empty())
            return {};

        if (into == PNG)
            return encodePng(decoded.rgb.data(), decoded.width, decoded.height);
        else
            return encodeJpeg(decoded.rgb.data(), decoded.width, decoded.height);
    }
