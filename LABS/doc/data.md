...
## Vibe File Format (.vibe)

### Purpose

The `.vibe` format stores style presets generated by the tune tool. It captures the **39 creative dials** (35 color/tone + 4 detail) but **excludes geometric dials** (user-controlled).

See [tune.md](./tune.md) for the three roles of tuning.
...
| `spsa_loss` | float | Final spectral loss (color/tone) |
| `edge_loss` | float | Final frequency loss (sharpness) |
| `spsa_iterations` | int | Number of SPSA iterations |
| `reference` | string | Optional: reference image filename |

### Dials Section

The `dials` section contains **39 creative dials** organized by module:

| Module | Dials | Optimized By |
|--------|-------|--------------|
| color_correction | 3 | SPSA (spectral) |
| tone_mapping | 5 | SPSA (spectral) |
| global_color | 3 | SPSA (spectral) |
| selective_color | 24 | SPSA (spectral) |
| detail | 4 | Edge (frequency) |
| **Total** | **39** | |

**NOT included:** `geometric` (6 dials) - user sets per image

### Differences from Pipe JSON

| Aspect | Pipe JSON | Vibe |
|--------|-----------|------|
| **Purpose** | Pipeline configuration | Style preset |
| **Dial count** | Variable (sparse) | 39 (no geometric) |
| **Dial storage** | Sparse (only set dials) | Dense (all 39 dials) |
| **Metadata** | Version, decoder | Losses, iterations |
| **Links** | Array of named links | Single flat dials object |

### Loading a Vibe

To apply a vibe to a new image:

```cpp
// Load vibe
json vibe = json::parse(readFile("style.vibe"));

// User sets geometry for this specific image
auto link = body.add("styled");
link.geometric().crop().top(user_crop);  // User-controlled
link.geometric().zoom().scale(user_zoom);
// ... other geometric dials ...

// Apply 35 color/tone dials from vibe
applyColorCorrection(link, vibe["dials"]["color_correction"]);
applyToneMapping(link, vibe["dials"]["tone_mapping"]);
applyGlobalColor(link, vibe["dials"]["global_color"]);
applySelectiveColor(link, vibe["dials"]["selective_color"]);

// Apply 4 detail dials from vibe
applyDetail(link, vibe["dials"]["detail"]);
```

### Validation

- **Version**: Must be "1.0"
- **Dials**: All 39 creative dials must be present (no geometric)
- **Values**: All in range [0.0, 1.0]
- **Meta fields**: timestamp, spsa_loss, edge_loss required
