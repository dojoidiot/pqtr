...
// Simple bilinear interpolation, RGGB pattern, with white balance
static void createGearTexture() {
    ...
    // Get white balance coefficients from metadata
    const float* wb_coeffs = nullptr;
    size_t wb_size = 0;
    if (g_state.gear_info) {
        wb_coeffs = g_state.gear_info->data("white_balance");
        wb_size = g_state.gear_info->size("white_balance");
    }
    ...
    for (int y = 0; y < out_h; y++) {
        for (int x = 0; x < out_w; x++) {
            ...
            float r  = g_state.bayer_data[by * w + bx];
            float gr = g_state.bayer_data[by * w + bx + 1];
            float gb = g_state.bayer_data[(by + 1) * w + bx];
            float b  = g_state.bayer_data[(by + 1) * w + bx + 1];

            // Average greens
            float g = (gr + gb) / 2.0f;

            // Apply white balance if available (R and B channels)
            if (wb_coeffs && wb_size >= 3) {
                r *= wb_coeffs[0];
                b *= wb_coeffs[2];
            }

            // Normalize to 0-255 (linear, no gamma)
            auto norm = [black, scale](float v) -> uint8_t {
...
static void startTunePipeline() {
    ...
    clearTasks();
    addTask("gear.load");
    addTask("pipe.post");   // Push preview PNG to BASE
    addTask("wgpu.open");
...
static void runTuneStep() {
    ...
    if (strcmp(task_name, "gear.load") == 0) {
        ...
        // Encode preview RGB to PNG in memory using stb_image_write
        struct PngBuffer {
            std::vector<uint8_t> data;
        };
        static PngBuffer png_buf;
        png_buf.data.clear();

        auto write_func = [](void* ctx, void* data, int size) {
            auto* buf = static_cast<PngBuffer*>(ctx);
            const uint8_t* bytes = static_cast<const uint8_t*>(data);
            buf->data.insert(buf->data.end(), bytes, bytes + size);
        };

        int result = stbi_write_png_to_func(write_func, &png_buf,
            g_state.preview_width, g_state.preview_height, 3,
            g_state.preview_data, g_state.preview_width * 3);

        if (result == 0 || png_buf.data.empty()) {
        ...
        char msg[64];
        snprintf(msg, sizeof(msg), "PNG %zu bytes", png_buf.data.size());
        setTaskStatus(step, TaskStatus::Running, msg);
        postNote("pipe.post", msg);

        // Build URL: POST /push?name={basename}&file={basename}.png
        static char post_url[512];
        snprintf(post_url, sizeof(post_url), "/push?name=%s&file=%s.png",
                 g_state.current_pipe, g_state.current_pipe);

        // Use static buffer for request data (like gear.load pattern)
        emscripten_fetch_attr_t attr;
        emscripten_fetch_attr_init(&attr);
        strcpy(attr.requestMethod, "POST");
        attr.attributes = EMSCRIPTEN_FETCH_LOAD_TO_MEMORY;
        attr.requestHeaders = g_auth_headers;
        attr.requestData = reinterpret_cast<const char*>(png_buf.data.data());
        attr.requestDataSize = png_buf.data.size();
        attr.userData = reinterpret_cast<void*>(static_cast<intptr_t>(step));
    ...
    switch (ctx->type) {
        case FetchType::Login: {
            ...
            if (strstr(buf, "\"ok\":true")) {
                ...
                strcpy(g_state.status_message, "Code sent! Check your email (or server terminal in test mode)");
                g_state.error_message[0] = '\0';
            } else {
                // Extract error message from JSON-RPC error response
                // Format: {"error":{"code":-32001,"message":"..."}}
        ...
        case FetchType::Verify: {
            ...
            if (strstr(buf, "\"error\"")) {
                extractJsonString(buf, "message", g_state.error_message, sizeof(g_state.error_message));
                if (g_state.error_message[0] == '\0') { // Fallback
        ...
        case FetchType::List: {
            ...
            if (fetch->status == 401 || fetch->status == 403 ||
                ...
                strcpy(g_state.status_message, "Your session has expired. Please login again.");
...
static void onFetchError(emscripten_fetch_t* fetch) {
    ...
    if (ctx->isValid()) {
        // Check for auth failure on protected endpoints
        if ((fetch->status == 401 || fetch->status == 403) &&
            ctx->type != FetchType::Login && ctx->type != FetchType::Verify) {
            LOG("Auth failed - redirecting to login");
            postNote("auth.expired", "");
            clearAuth();
            invalidateRequests();
            g_state.screen = Screen::Login;
...
static void render_login_screen() {
    ...
    // Error message (red) with guidance
    if (g_state.error_message[0] != '\0') {
    ...
    if (ImGui::Button(g_state.request_pending ? "Sending..." : "Continue", ImVec2(-1, 32)) || (enter_pressed && can_submit)) {
        g_state.error_message[0] = '\0';   // Clear error on new attempt
        g_state.status_message[0] = '\0';  // Clear status on new attempt
        sendLoginRequest();
    }
...
int main(int argc, char** argv) {
    ...
    // Palette - neutral darks (no tint, critical for photo work)
    ImVec4 bg      = ImVec4(0.08f, 0.08f, 0.09f, 1.0f);  // deepest
    ImVec4 panel   = ImVec4(0.12f, 0.12f, 0.13f, 1.0f);  // windows
    ImVec4 surface = ImVec4(0.18f, 0.18f, 0.19f, 1.0f);  // frames
    ImVec4 hover   = ImVec4(0.24f, 0.24f, 0.26f, 1.0f);
    ImVec4 active  = ImVec4(0.30f, 0.30f, 0.32f, 1.0f);
    ImVec4 accent  = ImVec4(0.35f, 0.55f, 0.90f, 1.0f);  // PQTR blue
    ...
    c[ImGuiCol_ChildBg]        = ImVec4(0, 0, 0, 0);  // transparent
