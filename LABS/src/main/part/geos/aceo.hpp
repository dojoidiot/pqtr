// aceo.hpp
// Internal: ACEO optimizer for color/tone dials
...
//
// Key features:
//   - Uses prior correlation matrix from etc/aceo.json
//   - Samples from multivariate normal using Cholesky decomposition
...
{
    // Variable dials for ACEO (excludes fixed: exposure, temperature, tint, white_point, black_point)
    // Maps to indices in the full 41-dial theta vector
    constexpr int ACEO_DIAL_COUNT = 36;

    // Mapping from ACEO dial index to full theta index
    // Excludes: 0 (exposure), 1 (temperature), 2 (tint), 8 (black), 9 (white)
    ...
    // Default parameters tuned for 36-dimensional dial optimization
    constexpr AceoParams ACEO_PARAMS_36D = {
        .lambda = 16,        // Population size (larger for better exploration)
        .mu = 8,             // Parents = lambda/2
        .sigma0 = 0.25f,     // Initial step size (25% of dial range, more exploration)
        .sigma_min = 0.005f, // Minimum step size
        .sigma_max = 0.5f,   // Maximum step size
        .c_sigma = 0.2f,     // Step size adaptation (slower)
        .d_sigma = 1.5f,     // Damping (more stable)
        .c_c = 0.1f,         // Cumulation rate
        .c_1 = 0.01f,        // Rank-one weight
        .c_mu = 0.03f        // Rank-mu weight (smaller for stability)
    };
    ...
    // Returns true if successful, matrix is 36x36 row-major
