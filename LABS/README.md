...
LABS is for professionals and enthusiasts who want full control over RAW processing. Styles created in LABS can be shared with PLAY users.

```
[PIPE] ──► [LABS] ──► Styles (.pipe.json)
             │              │
...
- **Produces**: `labs` executable, `.pipe.json` style files
...
When processing a RAW file, LABS produces these outputs:
...
|-----------|-------------|
| Sidecar Files | `<name>.labs.json` (project), `<name>.pipe.json` (pipe config) |
...
- [Specification](doc/labs.md) - Full application spec
