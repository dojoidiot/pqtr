...
This document defines data persistence formats for the PQTR:PIPE system. It covers:
...
## Camera Metadata Format (meta.json)

### Purpose

The `--fine` option saves camera metadata to `meta.json` for analysis and record-keeping. This captures EXIF and camera information from the RAW file.
...
```json
{
  ...
  "decoder": "raws_sony_arw2",
  "focal_length": "105",
  "height": "2624",
  "iso": "100",
  "lens_model": "E PZ 18-105mm F4 G OSS",
  "orientation": "1",
  "shutter_speed": "0.005",
  "white_level": "15360",
  "width": "3936"
...
| `bayer_pattern` | Bayer CFA pattern code |
...
| `shutter_speed` | Exposure time (seconds) |
| `white_level` | RAW white level (saturation) |
| `width` | Image width (pixels) |

### Usage

Generated by `tune --fine` alongside intermediate images:

```
<output>/
  meta.json      ← Camera metadata
  tune.jpg       ← Target image
  head.png       ← Target at working size
  body.png       ← Baseline output
  tail.png       ← Optimized output
  diff.png       ← Difference × 5
  tune.json      ← Dial settings + LUT
```
